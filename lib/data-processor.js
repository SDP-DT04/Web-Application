...
Proc.prototype.process_dataset = function (accel_data, tot_time,  callback) {
		...
		var velocity = midpoint(accel_data);
		var position = midpoint(velocity);
		max_bucket = Math.max.apply(Math, position);
		//console.log(max_bucket);
		position = apply_bucket_relationship(position);

		var time_diff = tot_time - time[time.length-1];
		time_diff = Math.floor(time_diff * 100); 

		var endPosition = position[position.length -1];
		for (i=0; i < time_diff; ++i)
		{
		 	position.push(endPosition); 	
		} 
